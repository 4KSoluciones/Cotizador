...
FORM f_pauta_variables_importes.
  ...
  DATA: lw_pauta TYPE ty_pauta_serv,
  ...
  IF gt_pauta_serv IS INITIAL. "Si no es inicial Cargó una pauta vieja

    SELECT *
      FROM ztpauta_serv
      INTO CORRESPONDING FIELDS OF TABLE gt_pauta_serv
     WHERE equipo      = wa_screen1-equipo
       AND marca       = wa_screen1-marca
       AND modelo      = wa_screen1-modelo
       AND modalidad   = wa_screen1-modalidad   "Operacion
       AND caja        = wa_screen1-caja        "Caja
       AND diferencial = wa_screen1-diferencial."Diferencial

    IF sy-subrc = 0.
      LOOP AT gt_pauta_serv ASSIGNING FIELD-SYMBOL(<fs_pauta>).
        lv_tabix = sy-tabix.
        "Filtra Lugar de Operacion Atributo 2
        FIND wa_screen1-lugar(2) IN <fs_pauta>-lugar.
        IF sy-subrc NE 0.
          DELETE gt_pauta_serv INDEX lv_tabix.
        ELSE.
...
*   >>> NUEVO: Descartar intervalos excluidos (ZTPARAM_COTSERV)
    READ TABLE gt_param INTO DATA(lw_interv_excl)
      WITH KEY tipo = 'INTERV_EXCLUIR'.
    IF sy-subrc = 0.
      IF lw_interv_excl-valor1 IS NOT INITIAL.
        DELETE gt_pauta_serv WHERE intervalo = lw_interv_excl-valor1.
      ENDIF.
      IF lw_interv_excl-valor2 IS NOT INITIAL.
        DELETE gt_pauta_serv WHERE intervalo = lw_interv_excl-valor2.
      ENDIF.
      IF lw_interv_excl-valor3 IS NOT INITIAL.
        DELETE gt_pauta_serv WHERE intervalo = lw_interv_excl-valor3.
      ENDIF.
      IF lw_interv_excl-valor4 IS NOT INITIAL.
        DELETE gt_pauta_serv WHERE intervalo = lw_interv_excl-valor4.
      ENDIF.
    ENDIF.
*   <<< FIN NUEVO
...
FORM f_obtener_variables.
  ...
  DATA: lv_modelo    TYPE zed_modelo,
        lv_intervalo TYPE zed_interv_mine.

  SELECT *
    FROM zttempario_serv
...
FORM f_importe_repuestos .
  ...
  REFRESH lr_matnr.
  LOOP AT gt_pauta_serv ASSIGNING FIELD-SYMBOL(<fs_pauta>)."INTO DATA(lw_pauta).
    lw_matnr-sign   = 'I'.
    lw_matnr-option = 'EQ'.
    PERFORM f_agregar_ceros CHANGING <fs_pauta>-matnr.
    lw_matnr-low    = <fs_pauta>-matnr.
...
FORM f_importe_implubri.
  ...
  SELECT *
    ...
    INTO TABLE @DATA(lt_mara)
     FOR ALL ENTRIES IN @gt_pauta_serv
   WHERE matnr EQ   @gt_pauta_serv-matnr
...
*            lv_total = lw_konp-kbetr * ( 1 - ( lv_dto_lubri ) / 100 ) * 100.
            ...
            wa_servicios-matnr     = lw_a519-matnr.
  ...
  "'CS03' "Maquinaria Construccion
  "'CS19' "Maquinaria Construccion
  "'CS35' "Camion
  "'CS84' "Agricola
...
* MANO DE OBRA
  ...
  IF sy-subrc = 0.
    ...
    SELECT *
      FROM konp
      INTO TABLE lt_konp
       FOR ALL ENTRIES IN lt_a544
     WHERE knumh = lt_a544-knumh.

    IF sy-subrc = 0.

*      REFRESH gt_servicios[].
      LOOP AT lt_konp INTO lw_konp.
        READ TABLE lt_a544 INTO DATA(lw_a544)
          WITH KEY knumh = lw_konp-knumh.
        IF sy-subrc = 0.
          CLEAR: wa_servicios, lv_total, v_tipo.
          lv_total = lw_konp-kbetr * 100.
          wa_manodeobra-importe_taller  = lv_total.
          wa_manodeobra-importe_terreno = lv_total.
          wa_manodeobra-sucursal        = wa_screen1-sucursal.
          wa_manodeobra-equipo          = wa_screen1-equipo.

*         COMISION
          CONCATENATE 'COMISION_' wa_a544-matnr
                 INTO v_tipo.

          READ TABLE gt_param INTO lw_param
            WITH KEY tipo = v_tipo.
          IF sy-subrc = 0.
            wa_comision-comision1 = lw_param-valor1.
            wa_comision-comision2 = lw_param-valor2.
            wa_comision-comision3 = lw_param-valor3.
            wa_comision-comision4 = lw_param-valor4.
          ENDIF.
  ...
  SELECT *
   ...
   WHERE kappl = 'V'
     ...
     AND vkorg = '1000'
     AND vtweg = 'SE'
     AND ( matnr = '1006298' OR "Alojamiento
           matnr = '1006299' OR "Desayuno
           matnr = '1006332' OR "Almuerzo
           matnr = '1006333' )  "Cena
     AND datab LT sy-datum
     AND datbi GT sy-datum.
  ...
  IF sy-subrc = 0.
    ...
    IF sy-subrc = 0.
      ...
      LOOP AT lt_a564 INTO lw_a564.
        READ TABLE lt_konp INTO lw_konp
          WITH KEY knumh = lw_a564-knumh.
        ...
        CASE lw_a564.
          WHEN '1006298'.
            IF wa_manodeobra-alojamiento IS INITIAL.
              wa_manodeobra-alojamiento = lv_total.
            ENDIF.
          WHEN '1006299'.
            IF wa_manodeobra-desayuno IS INITIAL.
              wa_manodeobra-desayuno    = lv_total.
            ENDIF.
          WHEN '1006332'.
            IF wa_manodeobra-almuerzo IS INITIAL.
              wa_manodeobra-almuerzo    = lv_total.
            ENDIF.
          WHEN '1006333'.
            IF wa_manodeobra-cena_comida IS INITIAL.
              wa_manodeobra-cena_comida = lv_total.
            ENDIF.
        ENDCASE.
...
FORM f_obtener_formula .
  ...
  DATA: lt_aceite     TYPE STANDARD TABLE OF ty_pauta_serv,
        lt_pauta_i    TYPE tyt_pauta_i,
        lt_repuesto   TYPE STANDARD TABLE OF ty_pauta_serv,
        lt_lubricante TYPE STANDARD TABLE OF ty_pauta_serv,

        lw_tempario   TYPE zttempario_serv,
        lw_aceite     TYPE ty_pauta_serv,
        lw_repuesto   TYPE ty_pauta_serv,
        lw_lubricante TYPE ty_pauta_serv,
        lw_cotizacion TYPE ztcoti_serv,
        lw_contrato   TYPE ty_val_contrato,
        lw_pauta_i    TYPE ty_pauta_i,
*        lw_comision   TYPE ztcomision,
        lw_intervalos TYPE ztcoti_interserv.
  ...
  DATA: lv_hora_fin       TYPE p DECIMALS 3,
        ...
        lv_tasa_mensu     TYPE ze_tasa_mensual, "n LENGTH 4,
        lv_precio_km      TYPE i,
        lv_precio_hs      TYPE i,
        lv_intervalo      TYPE zed_intervalo,
  ...
  FIELD-SYMBOLS: <f_field>       TYPE any,
                 <fs_struc>      TYPE any,
                 <fs_intervalos> TYPE ztcoti_interserv.
  ...
  MOVE-CORRESPONDING gt_pauta_serv TO lt_pauta_i.
  "Determina variable de análisis valuaciones.
  SORT lt_pauta_i BY intervalo ASCENDING.

  TRY.
      DATA(lv_interv) = lt_pauta_i[ 1 ]-intervalo.
  ...
  IF lv_interv NE 0. "Solo si intervalo tiene valor si es 0 no pasa
    ...
    REFRESH lt_aceite[].
    lt_aceite[] = gt_pauta_serv[].
    DELETE lt_aceite WHERE material NP 'MUESTRA DE ACEITE*'.

    REFRESH lt_repuesto[].
    lt_repuesto[] = gt_pauta_serv[].
    DELETE lt_repuesto WHERE material NP 'REPUESTO*'.

    REFRESH lt_lubricante[].
    lt_lubricante[] = gt_pauta_serv[].
    DELETE lt_lubricante WHERE material NP 'LUBRICACION*'.
    ...
    IF wa_screen1-chk_ipc IS INITIAL.
      ...
      IF sy-subrc = 0 AND wa_a544-matnr NE 'CS84'. "wa_screen1-equipo NE c_agricola.
    ...
    ELSE.
      ...
      IF wa_a544-matnr EQ 'CS84'. "wa_screen1-equipo EQ c_agricola.
        lv_dto_mo1 = '0%'.
        lv_dto_mo3 = '0%'.
        lv_dto_mo4 = '0%'.
      ELSE.
        ...
        IF sy-subrc = 0.
          lv_dto_mo1 = lw_param-valor1.
          lv_dto_mo3 = lw_param-valor3.
          lv_dto_mo4 = lw_param-valor4.
        ENDIF.
      ENDIF.
    ENDIF.
...
***************************************
    ...
    DO.
      ...
      LOOP AT gt_tempario INTO lw_tempario.
        IF ( lv_interv * lv_cant ) MOD lw_tempario-intervalo = 0.
      ...
      IF lv_dto_mo1 IS NOT INITIAL.
        lv_long = strlen( lv_dto_mo1 ) - 1.
        WRITE lv_dto_mo1(lv_long) TO lv_valor_d.
      ...
      ELSE.
        LOOP AT lt_aceite INTO lw_aceite. "MUESTRA DE ACEITE
          IF lw_aceite-intervalo NE 0.
            IF ( lv_interv * lv_cant ) MOD lw_aceite-intervalo = 0.
      ...
      ELSE.
        LOOP AT lt_repuesto INTO lw_repuesto.
          IF lw_repuesto-intervalo NE 0.
            IF ( lv_interv * lv_cant ) MOD lw_repuesto-intervalo = 0.
      ...
      ELSE.
        LOOP AT lt_lubricante INTO lw_lubricante.
          IF lw_lubricante-intervalo NE 0.
            IF ( lv_interv * lv_cant ) MOD lw_lubricante-intervalo = 0.
              READ TABLE gt_implubri INTO wa_implubri
      ...
      IF lv_tasa_lub(1) NE '0' AND lv_tasa_lub IS NOT INITIAL.
        lv_long = strlen( lv_tasa_lub ) - 1.
        WRITE lv_tasa_lub(lv_long) TO lv_valor_tr.
      ENDIF.

      lv_lub_ajust = lv_l * ( 1 + lv_valor_tr  / 100 ) ** ( lv_valor_p - 1 ).
      ...
      IF lv_cant EQ 1.
        lv_comision1 = wa_comision-comision1.
      ELSE.
        CLEAR lv_comision1.
      ENDIF.
      ...
      IF wa_screen1-uso_inicial IS INITIAL AND wa_a544-matnr EQ 'CS84' AND lv_cant = 1.
        lv_prepago1_acum = lv_prepago1_acum * 2.
      ENDIF.

*     Guardo Intervalos
      lw_intervalos-intervalo = lv_cant * lv_interv.
...
***************************************
    ...
    DO.
      ...
      IF lw_contrato-valor3 < ( lv_cant * lv_interv ) . EXIT. ENDIF.
      ...
      LOOP AT gt_tempario INTO lw_tempario.
        IF ( lv_interv * lv_cant ) MOD lw_tempario-intervalo = 0.
          "Se multiplica lw_tempario-cantidad * precio y sumarizar para obtener el valor
          lv_cantidad = lw_tempario-cantidad.
          ADD lv_cantidad TO lv_hora_fin. " importe de cada material
        ENDIF.
      ENDLOOP.
      ...
      IF lv_dto_mo3 IS NOT INITIAL.
        lv_long = strlen( lv_dto_mo3 ) - 1.
        WRITE lv_dto_mo3(lv_long) TO lv_valor_d.
      ENDIF.
      ...
      IF lv_cant EQ 1.
        lv_comision2 = wa_comision-comision2.
        lv_comision3 = wa_comision-comision3.
      ELSE.
      ...
      IF wa_screen1-uso_inicial IS INITIAL AND wa_a544-matnr EQ 'CS84' AND lv_cant = 1.
        lv_prepago2_acum = lv_prepago2_acum * 2.
        lv_prepago3_acum = lv_prepago3_acum * 2.
      ENDIF.
...
***************************************
    ...
    DO.
      ...
      IF lw_contrato-valor4 < ( lv_cant * lv_interv ) . EXIT. ENDIF.
      "DURACION EN MES
      IF ( ( lv_interv * lv_cant ) - wa_screen1-uso_inicial <= 0 ).
        lv_perio_mes4 = 1.
      ELSE.
      ...
      IF lv_dto_mo4 IS NOT INITIAL.
        lv_long = strlen( lv_dto_mo4 ) - 1.
        WRITE lv_dto_mo4(lv_long) TO lv_valor_d.
      ENDIF.
      ...
      IF lv_cant EQ 1.
        lv_comision4 = wa_comision-comision4.
      ELSE.
        CLEAR lv_comision4.
      ENDIF.
      ...
      IF wa_screen1-uso_inicial IS INITIAL AND wa_a544-matnr EQ 'CS84' AND lv_cant = 1.
        lv_prepago4_acum = lv_prepago4_acum * 2.
      ENDIF.
...
*   PROCESO FINAL
***************************************
    READ TABLE gt_pauta_servsave  INTO DATA(lw_pauta_s) INDEX 1.
...
*   DESCUENTO CLIENTE
    ...
    IF sy-subrc = 0.
      REPLACE ALL OCCURRENCES OF '%' IN lw_desc_cli-valor1 WITH ''.
      REPLACE ALL OCCURRENCES OF '%' IN lw_desc_cli-valor2 WITH ''.
      REPLACE ALL OCCURRENCES OF '%' IN lw_desc_cli-valor3 WITH ''.
      REPLACE ALL OCCURRENCES OF '%' IN lw_desc_cli-valor4 WITH ''.
      lv_prepago1_acum = lv_prepago1_acum - ( lv_prepago1_acum * lw_desc_cli-valor1 / 100 ).
      lv_prepago2_acum = lv_prepago2_acum - ( lv_prepago2_acum * lw_desc_cli-valor2 / 100 ).
      lv_prepago3_acum = lv_prepago3_acum - ( lv_prepago3_acum * lw_desc_cli-valor3 / 100 ).
      lv_prepago4_acum = lv_prepago4_acum - ( lv_prepago4_acum * lw_desc_cli-valor4 / 100 ).
    ENDIF.
    ...
    IF sy-subrc EQ 0.
      DATA(lv_k1000) = lw_desc_cli2-valor1.
*     Cuando el check IPC se encuentra seleccionado descuentos y tasas van en 0
      CLEAR lv_valor_tr.
*      lv_long = strlen( lv_k1000 ) - 1.
*      WRITE lv_k1000(lv_long) TO lv_tasa_mensu.
      WRITE lv_k1000 TO lv_tasa_mensu.
    ENDIF.
...
*   CASILLA DE AJUSTE de descuentos
    IF wa_screen1-chk_desc = 'X'.
      IF v_rb_dto$ = 'X'.
        lw_cotizacion-descuento_peso = wa_screen1-descuento_peso * 100. "Se multiplica por 100 ya es toma 2 digitos menos en la pantalla
...
*   TELEMETRIA
      ...
      CASE wa_a544-matnr.
        ...
        WHEN 'CS35'. "'CAMIONES'.
          ...
          ELSEIF wa_screen1-marca = 'RENAULT'.
            IF wa_screen1-uso_inicial IS INITIAL.
              READ TABLE gt_param INTO lw_param
                WITH KEY tipo = 'TELEM_RENAULT_NEW'.
              IF sy-subrc = 0.
                lv_prepago1_acum = lv_prepago1_acum + lw_param-valor1.
                lv_prepago2_acum = lv_prepago2_acum + lw_param-valor2.
                lv_prepago3_acum = lv_prepago3_acum + lw_param-valor3.
            ...
            ELSE.
              READ TABLE gt_param INTO lw_param
                WITH KEY tipo = 'TELEM_RENAULT_OLD'.
              IF sy-subrc = 0.
                lv_prepago1_acum = lv_prepago1_acum + lw_param-valor1.
                lv_prepago2_acum = lv_prepago2_acum + lw_param-valor2.
                lv_prepago3_acum = lv_prepago3_acum + lw_param-valor3.
          ...
          ENDIF.
        WHEN 'CS03' OR 'CS19'. "'MAQUINARIA'.
          READ TABLE gt_param INTO lw_param
            WITH KEY tipo = 'TELEM_MAQUINARIA'.
          IF sy-subrc = 0.
            lv_prepago1_acum = lv_prepago1_acum + lw_param-valor1.
            lv_prepago2_acum = lv_prepago2_acum + lw_param-valor2.
            lv_prepago3_acum = lv_prepago3_acum + lw_param-valor3.
    ...
    IF wa_screen1-modalidad EQ 'HORAS' OR
       wa_screen1-modalidad EQ 'HORA' OR
       wa_screen1-modalidad EQ 'HS'.
      lv_coef_km = 1.
    ELSE.
      lv_coef_km = 1000.
    ENDIF.
...
*   CALCULO EN CUOTA Y POR USO (AJUSTADO O NO)
    ...
    IF wa_screen1-chk_ipc IS INITIAL. "CALCULO CUOTA
      ...
      IF wa_screen1-chk_poruso = 'X'. "CALCULO POR USO KM/HS
        ...
        lv_valor_hs1 = lv_prepago1_acum / lw_contrato-valor1. "lv_cuota_t1 * lv_0coma1 / lv_coef_km.
        WRITE lv_valor_hs1 TO lw_cotizacion-val_kmhs1.

        lv_valor_hs2 = lv_prepago2_acum / lw_contrato-valor2. "lv_cuota_t2 * lv_0coma1 / lv_coef_km.
        WRITE lv_valor_hs2 TO lw_cotizacion-val_kmhs2.

        lv_valor_hs3 = lv_prepago3_acum  / lw_contrato-valor3. "lv_cuota_t3 * lv_0coma1 / lv_coef_km.
        WRITE lv_valor_hs3 TO lw_cotizacion-val_kmhs3.

        lv_valor_hs4 = lv_prepago4_acum  / lw_contrato-valor4. "lv_cuota_t4 * lv_0coma1 / lv_coef_km.
    ...
    lw_cotizacion-matnr        = wa_a544-matnr.
    ...
    lw_cotizacion-comision1     = wa_comision-comision1.
    lw_cotizacion-comision2     = wa_comision-comision2.
    lw_cotizacion-comision3     = wa_comision-comision3.
    lw_cotizacion-comision4     = wa_comision-comision4.
...
**  ADD 1 TO lv_col.
**  lw_fieldcat-tabname   = 'GT_PAUTA'.
**  lw_fieldcat-fieldname = 'ATRIBUTO1'.
**  lw_fieldcat-scrtext_s = lv_atributo1." 'Operacion'.
**  lw_fieldcat-scrtext_m = lv_atributo1.
**  lw_fieldcat-scrtext_l = lv_atributo1.
**  lw_fieldcat-col_pos   = lv_col.
**  lw_fieldcat-key       = space.
**  APPEND lw_fieldcat TO gt_fieldcat.
**
...
**  ADD 1 TO lv_col.
**  lw_fieldcat-tabname   = 'GT_PAUTA'.
**  lw_fieldcat-fieldname = 'ATRIBUTO2'.
**  lw_fieldcat-scrtext_s = lv_atributo2." 'Lugar de Ejecución'.
**  lw_fieldcat-scrtext_m = lv_atributo2.
**  lw_fieldcat-scrtext_l = lv_atributo2.
**  lw_fieldcat-col_pos   = lv_col.
**  lw_fieldcat-key       = space.
**  APPEND lw_fieldcat TO gt_fieldcat.
**
...
**  ADD 1 TO lv_col.
**  lw_fieldcat-tabname   = 'GT_PAUTA'.
**  lw_fieldcat-fieldname = 'ATRIBUTO3'.
**  lw_fieldcat-scrtext_s = lv_atributo3." 'Caja'.
**  lw_fieldcat-scrtext_m = lv_atributo3.
**  lw_fieldcat-scrtext_l = lv_atributo3.
**  lw_fieldcat-col_pos   = lv_col.
**  lw_fieldcat-key       = space.
**  APPEND lw_fieldcat TO gt_fieldcat.
**
...
**  ADD 1 TO lv_col.
**  lw_fieldcat-tabname   = 'GT_PAUTA'.
**  lw_fieldcat-fieldname = 'ATRIBUTO4'.
**  lw_fieldcat-scrtext_s = lv_atributo4." 'Diferencial'.
**  lw_fieldcat-scrtext_m = lv_atributo4.
**  lw_fieldcat-scrtext_l = lv_atributo4.
**  lw_fieldcat-col_pos   = lv_col.
**  lw_fieldcat-key       = space.
**  APPEND lw_fieldcat TO gt_fieldcat.
**
...
**  ADD 1 TO lv_col.
**  lw_fieldcat-tabname   = 'GT_PAUTA'.
**  lw_fieldcat-fieldname = 'ATRIBUTO5'.
**  lw_fieldcat-scrtext_s = lv_atributo5." 'Atributo5'.
**  lw_fieldcat-scrtext_m = lv_atributo5.
**  lw_fieldcat-scrtext_l = lv_atributo5.
**  lw_fieldcat-col_pos   = lv_col.
**  lw_fieldcat-key       = space.
...
**  ADD 1 TO lv_col.
**  lw_fieldcat-tabname   = 'GT_PAUTA'.
**  lw_fieldcat-fieldname = 'ATRIBUTO6'.
**  lw_fieldcat-scrtext_s = lv_atributo6." 'Atributo6'.
**  lw_fieldcat-scrtext_m = lv_atributo6.
**  lw_fieldcat-scrtext_l = lv_atributo6.
**  lw_fieldcat-col_pos   = lv_col.
**  lw_fieldcat-key       = space.
**  lw_fieldcat-no_out    = 'X'.
**  APPEND lw_fieldcat TO gt_fieldcat.
...
FORM f_procesa_pauta CHANGING v_numeropauta TYPE zed_numeropauta.
  ...
  DATA: lw_pauta_save TYPE ztpauta_servsave,
        lw_pauta      TYPE ty_pauta_serv.

  IF gt_pauta_serv IS NOT INITIAL.

    CLEAR v_numeropauta.
    SELECT SINGLE MAX( numero_pauta )
      FROM ztpauta_servsave
      INTO v_numeropauta
    ...
    LOOP AT gt_pauta_serv INTO lw_pauta.
      CLEAR lw_pauta_save.
      ...
      lw_pauta_save-intervalo       = lw_pauta-intervalo.
...
*            lw_pauta_save-dto_aplicado = lw_repuestos-descuento.
      ...
      MODIFY ztpauta_servsave FROM lw_pauta_save.
    ENDLOOP.
...
**      lw_pauta-atributo1       = lw_pauta_save-atributo1.
**      lw_pauta-atributo2       = lw_pauta_save-atributo2.
**      lw_pauta-atributo3       = lw_pauta_save-atributo3.
**      lw_pauta-atributo4       = lw_pauta_save-atributo4.
**      lw_pauta-atributo5       = lw_pauta_save-atributo5.
**      lw_pauta-atributo6       = lw_pauta_save-atributo6.
...
**      FROM ztrodado
**      INTO @DATA(lw_rodado)
**     WHERE modelo = @lw_pauta_save-modelo.
**
**    wa_screen1-equipo      = lw_rodado-equipo.
**    wa_screen1-marca       = lw_rodado-marca.
**    wa_screen1-kunnr       = lw_pauta_save-cliente.
**    wa_screen1-modelo      = lw_pauta_save-modelo.
**    wa_screen1-atr1        = lw_pauta_save-atributo1.
**    wa_screen1-atr2        = lw_pauta_save-atributo2.
**    wa_screen1-atr3        = lw_pauta_save-atributo3.
**    wa_screen1-atr4        = lw_pauta_save-atributo4.
**    wa_screen1-atr5        = lw_pauta_save-atributo5.
**    wa_screen1-atr6        = lw_pauta_save-atributo6.
...
*&---------------------------------------------------------------------*
FORM f_crear_fieldcat   USING pt_i_alv TYPE tyt_ztcoti_serv
...
FORM f_cargar_cotizacion_vieja .
  ...
  DATA: lv_answer     TYPE c,
        lw_cotizacion TYPE ztcoti_serv,
        lv_cliente    TYPE ztcoti_serv-cliente.
...
FORM user_command_int USING pv_ucomm    LIKE sy-ucomm
  ...
  DATA: lw_pauta     TYPE ty_pauta_serv,
        lw_new_pauta TYPE ty_pauta_serv.
...
FORM f_crear_pedido.
  ...
  DATA: lt_items                TYPE STANDARD TABLE OF bapisditm,
        ...
        lt_intervalos           TYPE STANDARD TABLE OF ztcoti_interserv,
...
* Org. ventas
  lw_order_header-sales_org   = '1000'.
  lw_order_header_x-sales_org = c_mark.

* Canal distr.
  lw_order_header-distr_chan   = 'VS'.
  lw_order_header_x-distr_chan = c_mark.

* Sector.
  lw_order_header-division   = 'R0'.
...
* Solicitante
  ...
  lw_order_partners-partn_role  = 'AG'.
...
* Textos
  ...
  IF v_auart NE 'ZPPE'. "Leasing hereda de la factura los textos
...
*************    SALES ORDER ITEM   ***************
  ...
  LOOP AT lt_intervalos INTO lw_intervalos.
    ...
    CASE v_duracion.
      WHEN wa_cotizacion-duracion1.
        lv_precio_int = lw_intervalos-prepago1.
      WHEN wa_cotizacion-duracion2.
        lv_precio_int = lw_intervalos-prepago2.
      WHEN wa_cotizacion-duracion3.
        lv_precio_int = lw_intervalos-prepago3.
      WHEN wa_cotizacion-duracion4.
        lv_precio_int = lw_intervalos-prepago4.
    ENDCASE.
...
*   Condiciones
    lw_order_conditions_in-cond_type  = 'ZFLE'.
...
* Call bapi
  ...
  IF lv_vbeln IS INITIAL.
    ...
    LOOP AT lt_return INTO lw_return WHERE type EQ 'E'.
      lw_bapiret2-type       = 'E'.
      lw_bapiret2-id         = 'ZSD'.
      lw_bapiret2-number     = 035.
      lw_bapiret2-message = lw_return-message_v1.
      lw_bapiret2-message_v2 = lw_return-message_v2.
      lw_bapiret2-message_v3 = lw_return-message_v3.
      lw_bapiret2-message_v4 = lw_return-message_v4.
  ...
  ELSE. "Pedido Creado con Éxito
    ...
    MODIFY ztcoti_serv FROM wa_cotizacion.
    ...
    LOOP AT lt_intervalos INTO lw_intervalos.
      ...
      ENDIF.
      MODIFY ztcoti_interserv FROM lw_intervalos.
    ENDLOOP.
...
FORM f_graba_cotizacion.
  ...
  FIELD-SYMBOLS <fs_intervalos> TYPE ztcoti_interserv.
  ...
  IF v_answer EQ '1'.
    ...
    IF sy-subrc = 0.
      ...
      MODIFY ztcoti_serv FROM lw_cotizacion.
      ...
      IF gt_intervalos IS NOT INITIAL.
        ...
        MODIFY ztcoti_interserv FROM TABLE gt_intervalos.
  ...
  v_active9000 = 'X'.
  REFRESH: gt_pauta_serv, gt_repuestos.
...
FORM f_proceso_tabla_detallecm  CHANGING pt_detalle TYPE ztt_detallecm.
  ...
  DATA: lt_pauta   TYPE STANDARD TABLE OF ty_pauta_serv,
        lw_detalle TYPE zts_detallecm,
        lv_cant    TYPE sy-tabix.

  lt_pauta[] = gt_pauta_serv[].
  SORT lt_pauta BY intervalo ASCENDING.
  READ TABLE lt_pauta INTO DATA(lw_pauta2) INDEX 1.
  IF sy-subrc = 0.
    DATA(lv_intervalo) = lw_pauta2-intervalo.
  ENDIF.
  LOOP AT gt_pauta_serv INTO DATA(lw_pauta).
    AT FIRST.
      lw_detalle-servicio    = ''.
    ...
    ADD 1 TO lv_cant.
    IF ( lv_intervalo * lv_cant ) MOD lw_pauta-intervalo = 0.
...
* Macro para leer parámetros
  DEFINE get_param.
    READ TABLE lt_param_oc INTO ls_param_oc
      WITH KEY tipo = &1.
    IF sy-subrc = 0.
      &2 = ls_param_oc-valor.
    ELSE.
...
DATA: lt_param_oc TYPE STANDARD TABLE OF ty_param_oc,
        ls_param_oc TYPE ty_param_oc.
    ...
    IF sy-subrc = 0.
      SELECT *
       ...
       WHERE numero_pauta EQ @wa_cotizacion-numero_pauta
         ...
         AND intervalo    LE @<fs_alvoc>-intervalo.
    ENDIF.
  ENDIF.
...
get_param 'OC_BUKRS' lv_bukrs '1000'.
  ...
  get_param 'OC_EKORG' lv_ekorg '1000'.
  get_param 'OC_EKGRP' lv_ekgrp '107'.
  get_param 'OC_WAERS' lv_waers 'CLP'.
  get_param 'OC_KNTTP' lv_knttp 'S'.
  get_param 'OC_MWSKZ' lv_mwskz 'C1'.
  get_param 'OC_GLACCOUNT' lv_glaccount '0021110018'.

  lw_header-comp_code  = lv_bukrs.      " Antes: '1000'
  lw_header-doc_type   = lv_bsart.      " Antes: 'ZPP2'
  lw_header-creat_date = sy-datum.
  lw_header-vendor     = lv_lifnr.      " Antes: 'RSA0'
  lw_header-purch_org  = lv_ekorg.      " Antes: '1000'
  lw_header-pur_group  = lv_ekgrp.      " Antes: '107'
  lw_header-currency   = lv_waers.      " Antes: 'CLP'
...
*&---------------------------------------------------------------------*
*&      Form  F_VALIDA_INICIO
*&---------------------------------------------------------------------*
FORM f_valida_inicio .

  v_valida_inicio = 'X'.

  IF wa_screen1-kunnr IS INITIAL.
    MESSAGE 'Debe ingresar Codigo de Cliente' TYPE 'S' DISPLAY LIKE 'E'.
  ELSE.
    IF wa_screen1-equipo IS INITIAL.
      MESSAGE 'Debe ingresar Equipo' TYPE 'S' DISPLAY LIKE 'E'.
    ELSE.
      IF wa_screen1-marca IS INITIAL.
        MESSAGE 'Debe ingresar Marca' TYPE 'S' DISPLAY LIKE 'E'.
      ELSE.
        IF wa_screen1-modelo IS INITIAL.
          MESSAGE 'Debe ingresar Modelo' TYPE 'S' DISPLAY LIKE 'E'.
        ELSE.
          IF wa_screen1-modalidad IS INITIAL.
            MESSAGE 'Debe ingresar Modalidad (Kilómetros-Horas)' TYPE 'S' DISPLAY LIKE 'E'.
          ELSE.
            IF wa_screen1-sucursal IS INITIAL.
              MESSAGE 'Debe ingresar Código de Sucursal' TYPE 'S' DISPLAY LIKE 'E'.
            ELSE.
              IF wa_screen1-uso IS INITIAL.
                v_active9000 = 'X'.
                MESSAGE 'Debe ingresar Intervalo de Uso' TYPE 'S' DISPLAY LIKE 'E'.
              ELSE.
                IF wa_screen1-lugar IS INITIAL.
                  v_active9000 = 'X'.
                  MESSAGE 'Debe ingresar lugar (Taller-Terreno)' TYPE 'S' DISPLAY LIKE 'E'.
                ELSE.
                  IF wa_screen1-equipo EQ '1009'.
                    IF wa_screen1-caja IS INITIAL.
                      v_active9000 = 'X'.
                      MESSAGE 'Debe ingresar Caja' TYPE 'S' DISPLAY LIKE 'E'.
                    ELSE.
                      IF wa_screen1-diferencial IS INITIAL.
                        v_active9000 = 'X'.
                        MESSAGE 'Debe ingresar Diferencial' TYPE 'S' DISPLAY LIKE 'E'.
                      ELSE.
                        CLEAR v_valida_inicio.
                      ENDIF.
                    ENDIF.
                  ELSE.
                    CLEAR v_valida_inicio.
...
*&---------------------------------------------------------------------*
*&      Form  F_AGREGAR_CEROS
*&---------------------------------------------------------------------*
FORM f_agregar_ceros CHANGING p_valor TYPE any.
